...
# from lxrt.modeling_learnedPEmixer import BertLayerNorm, GeLU  # learned PEmixer
...
class BanFusion(nn.Module):
    def __init__(self, dataset, b_att, b_net, q_prj, c_prj, counter, gamma, omega):
        ...
        self.dataset = dataset  # GQAFeatureDataset
        self.glimpse = gamma  # 1
        self.omega = omega  # 0.001
        self.v_att = b_att  # BiAttention network
    ...
    def forward(self, mod1, b, mod2):  # mod1.device: cuda, b.device: cuda, mod2.device: cuda
        """Forward
        ...
        b_emb = [0] * self.glimpse  # [0]
        # att: logits after softmax, logits: matmul tensor of mod1 and mod2
        att, logits = self.v_att.forward_all(mod1, mod2)  # b x g x v x q

        for g in range(self.glimpse):
            b_emb[g] = self.b_net[g].forward_with_weights(mod1, self.omega * mod2, att[:, g, :, :])  # weighting map Phi'
            mod1 = self.omega * self.q_prj[g](b_emb[g].unsqueeze(1)) * mod1 + mod1  # information filtering, Phi=(Phi'*1.T) * tau(f) + tau(f)
...
class Lxmert_Model(nn.Module):
    ...
    def forward(self, v, b, q, args):
        q_emb, v, logits = self.lxrt_encoder(q, (v, b[:, :, :4]), args)
        return q_emb, v, self.logit_fc(logits)  # question_embedding, visual_embedding, output of 2 fcl
...
class CFRF_Model(nn.Module):
    def __init__(self, dataset, args, lxmert_encoder, w_emb, q_emb, sw_emb, s_emb, ew_emb, qe_joint, vs_joint, vq_joint,
        ...
        self.adapted_w = nn.Parameter(torch.ones(2, dataset.num_ans_candidates))  # W, weight matrix
        self.classifier = classifier


    def forward(self, v, b, q, s, e, w, args):
        # Question embedding
        w_emb = self.w_emb(q)
        ...
        # Lxmert encoder
        _, _, lxmert_logit = self.lxmert_encoder(v, b, s, args)

        # Question-entity
        q_emb = self.qe_joint(q_emb, b, e_emb)  # BanFusion, apply information filtering to the question embedding

        # Image-semantic
        v = self.vs_joint(s_emb, b, v)  # BanFusion, apply information filtering to the image embedding

        # Image question
        q_emb = self.vq_joint(q_emb, b, v)  # BanFusion, fine-grained learning
...
def build_ban_fusion(dataset, args, dim_1, dim_2, gamma, omega, priotize_using_counter=False):
    ...
    for i in range(args.gamma):  # args.gamma: 2
...
def build_CFRF_Model(dataset, args):
    ...
    w_emb = WordEmbedding(dataset.dictionary.ntoken, 300, .0, args.op)  # ntoken=2931, args.op='c'
