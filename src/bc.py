...
class BCNet(nn.Module):
    ...
    """
    # zhd-2025/04/13: Correspond to the information filtering
    def __init__(self, v_dim, q_dim, h_dim, h_out, act='ReLU', dropout=[.2,.5], k=1):   # k = 3
        ...
        if None == h_out:
            ...
            # initialise the tensor by filling it with values drawn from a standard normal(Gaussian) distribution
    ...
    def forward(self, v, q):
        ...
        # epoch 1, time: 157.84
        elif self.h_out <= self.c:  # self.h_out: 1; self.c: 32
            v_ = self.dropout(self.v_net(v)).unsqueeze(1)  # Tensor: (256,1,12,1024)
            q_ = self.q_net(q)  # Tensor: (256,7,1024)
            """
            FCNet(
              (main): Sequential(
            ...
            """
            # h_: Tensor: (256,1,12,1024); v_: Tensor: (64, 1, 12, 1024); self.h_mat: Tensor: (1, 1, 1, 1024)
            h_ = v_ * self.h_mat # broadcast, b x h_out x v x h_dim
            logits = torch.matmul(h_, q_.unsqueeze(1).transpose(2,3)) # b x h_out x v x q Tensor: (256, 1, 12, 7)
    ...
    def forward_with_weights(self, v, q, w):
        v_ = self.v_net(v).transpose(1,2).unsqueeze(2) # b x d x 1 x v (256,1024,1,12)
        q_ = self.q_net(q).transpose(1,2).unsqueeze(3) # b x d x q x 1 (256,1024,7,1)
        # VWQ, this step is applying the biattention
        ...
        logits = logits.squeeze(3).squeeze(2)  # b x d, (256,1024)
        if 1 < self.k:
